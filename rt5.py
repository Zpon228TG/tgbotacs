...
def admin_menu(message):
    ...
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add('➕ Выдать доступ', '🗑️ Отозвать доступ', '🔙 Назад','📸 Добавить расписание как фото', '🗑️ Удалить расписание', '📸 Добавить именинников',  '➕ Добавить администратора', '🗑️ Удалить администратора')
...
def confirm_deletion(message, month):
    if message.text == 'Да':
        data = load_data()
        if month in data['birthdays']:
            # Удаление файла с диска
            file_path = data['birthdays'][month]
            ...
            bot.send_message(message.chat.id, f"Фото именинников для {month.capitalize()} не найдено.")
    else:
