...
def add_birthdays_photo(message):
    if not is_moderator(message.from_user.id):
        bot.send_message(message.chat.id, "У вас нет доступа к этой функции.")
        return

    msg = bot.send_message(message.chat.id, "Отправьте фото именинников:")
    bot.register_next_step_handler(msg, process_birthdays_photo)

def process_birthdays_photo(message):
    if message.photo:
        month = datetime.datetime.now().strftime("%B").lower()
        file_info = bot.get_file(message.photo[-1].file_id)
        file_path = file_info.file_path
        file = bot.download_file(file_path)
        file_name = os.path.join(BIRTHDAYS_PATH, f'{month}.jpg')
        with open(file_name, 'wb') as f:
            f.write(file)
        data = load_data()
        data['birthdays'][month] = file_name
        save_data(data)
...
def delete_birthdays_photo(message):
    if not is_moderator(message.from_user.id):
        bot.send_message(message.chat.id, "У вас нет доступа к этой функции.")
        return
    ...
    if month in data['birthdays']:
        ...
        save_data(data)
        bot.send_message(message.chat.id, "Фото именинников успешно удалено.")
    else:
        bot.send_message(message.chat.id, "Фото именинников для этого месяца отсутствует.")
