...
def start(message):
    ...
    add_user(user_id)
    bot.send_message(user_id, "Привет! Выберите действие:", reply_markup=main_keyboard())
...
@bot.message_handler(func=lambda message: str(message.chat.id) in [uid for uid in users_data] and message.text in users_data and str(message.chat.id) == ADMIN_ID)
